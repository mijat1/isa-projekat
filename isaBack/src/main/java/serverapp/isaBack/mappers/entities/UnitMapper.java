...
public class UnitMapper {
	...
	public UnspecifiedDTO<BoatDTO> MapBoatToBoatDTO(Boat boat,double avgGrade) throws IOException{
		...
		byte[] imageBytes = extractBytes(basePath+"\\src\\main\\java\\serverapp\\isaBack\\images\\" + boat.getAlbum().getImages().get(0).getFileName());
	...
	public UnspecifiedDTO<CottageDTO> MapCottageToCottageDTO(Cottage cottage,double avgGrade) throws IOException{
		...
		byte[] imageBytes = extractBytes(basePath+"\\src\\main\\java\\serverapp\\isaBack\\images\\" + cottage.getAlbum().getImages().get(0).getFileName());
